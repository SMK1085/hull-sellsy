...
export const server = (app: Application): Application => {
  ...
  const loggerOptions: LoggerOptions = {
    ...
    defaultMeta: {
      service: process.env.LOG_SERVICENAME || "hull-sellsy",
  ...
  app.get("/webhookurl", cors(), actions.webhookUrl());
  app.get(
    "/meta/identity/:objectType/:direction",
    cors(),
    actions.metaIdentity(),
  );
  app.get("/meta/fields/:objectType/:direction", cors(), actions.metaFields());
  app.use("/fetch/:objectType/:fetchType", cors(), actions.fetch());

  // Webhook endpoints
  app.post("/webhook", urlencoded(), actions.webhook());
